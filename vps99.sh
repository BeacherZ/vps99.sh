...
# 2. 内存：zRAM 与物理 Swap 双层策略，zRAM 压缩优先，磁盘 Swap 安全兜底。
...
# 收尾进度条：从 current_fill 冲到 100%，1 秒内完成
progress_finish() {
    start_fill="$1"
...
printf " - 内存优化 : zRAM 与物理 Swap 双层策略，zRAM 压缩优先，磁盘 Swap 安全兜底\n"
...
# swappiness：有物理 swap 压低，仅 zRAM 调高
...
if [ "$mem_enter" = "y" ] || [ "$mem_enter" = "Y" ]; then
    ...
    if [ -n "$REPORT_ZRAM" ] || [ -n "$REPORT_PHY" ]; then
        ...
        sysctl -p >/dev/null 2>&1
        printf "  ${GREEN}> swappiness 已设为 %s${NC}\n" "$FINAL_SWAPPINESS"
    fi
fi
...
printf " [内存优化] 物理swap : ${REPORT_PHY:-${YELLOW}保持现状${NC}}\n"
