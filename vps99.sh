...
fq_status=$(sysctl net.core.default_qdisc 2>/dev/null | awk '{print $3}')
...
[ $HAS_BBR -eq 1 ] && printf " - 网络算法 : ${GREEN}BBR+FQ 已激活${NC}\n" || printf " - 网络算法 : ${YELLOW}未开启${NC}\n"
...
if [ $HAS_BBR -eq 1 ]; then
    read -p "网络算法 : BBR+FQ 已激活，是否覆盖配置? [y/N]: " bbr_confirm
else
