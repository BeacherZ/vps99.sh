...
CUR_TZ=$(cat /etc/timezone 2>/dev/null || timedatectl show -p Timezone --value 2>/dev/null || echo "未知")
...
[ $HAS_BBR -eq 1 ] && printf " - 网络算法 : ${GREEN}BBR 已激活${NC}\n" || printf " - 网络算法 : ${YELLOW}未开启${NC}\n"
...
printf " - 网络算法 : 开启 TCP-BBR 拥塞控制，提升网络吞吐并降低延迟\n"
...
printf " - 即时清理 : 立即释放包管理器缓存与系统日志\n"
printf "${GREEN}================================================================================${NC}\n\n"

# 4.1 BBR
if [ $HAS_BBR -eq 1 ]; then
    read -p "网络算法 : BBR 已激活，是否覆盖配置? [y/N]: " bbr_confirm
else
    read -p "网络算法 : 是否开启 TCP-BBR? [y/N]: " bbr_confirm
fi
...
if [ "$bbr_confirm" = "y" ] || [ "$bbr_confirm" = "Y" ]; then
    ...
    printf "  ${GREEN}> BBR 已配置，重启后生效${NC}\n"
    REPORT_BBR="${GREEN}已开启${NC}"
    NEED_REBOOT=1
fi
...
printf " [网络算法] BBR      : ${REPORT_BBR:-${YELLOW}保持现状${NC}}\n"
