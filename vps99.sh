...
if [ "$docker_enter" = "y" ] || [ "$docker_enter" = "Y" ]; then
    ...
    case "$docker_choice" in
        1)
            printf "  正在安装/更新 Docker & Compose，预计1-3分钟，请稍候...\n"
            if [ "$OS" = "Debian" ]; then
                fix_dpkg
            fi
            if [ "$OS" = "Alpine" ]; then
            ...
            if command -v docker >/dev/null 2>&1; then
                ...
                if [ "$HAS_DOCKER" -eq 1 ] && [ "$NEW_DOCKER_VER" = "$DOCKER_VER" ] && [ "$NEED_UPDATE" -eq 1 ]; then
                    printf "  ${RED}> 更新失败，版本未变 (${NEW_DOCKER_VER})，请检查 apt 源${NC}\n"
                    REPORT_DOCKER="${RED}更新失败 (${NEW_DOCKER_VER})${NC}"
                else
